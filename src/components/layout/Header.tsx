...
export default function Header({heroSize = ''}: {heroSize?: string}) {
  ...
  return (
    <>
      {openDropdownKey && (
        <div className="fixed inset-0 z-10" onClick={closeDropdowns} onMouseEnter={closeDropdowns}>
          <div
            className={`
              pointer-events-none
              ...
              {MAIN_NAV.map((item) => {
                ...
                return (
                  <div
                    ...
                    onMouseEnter={() => item.children && setOpenDropdownKey(item.key)}
                    onMouseLeave={() => item.children && setOpenDropdownKey(null)}
                  >
                    <Link
                      href={item.href}
                  ...
                  <div key={item.key} className="flex flex-col gap-1">
                    <Link
                      ...
                      className={`text-xs font-semibold uppercase tracking-[0.18em] ${
                        isActive(item.href) ? 'text-[#68B52F]' : 'text-[#1C2537] hover:text-[#68B52F]'
                      }`}
                    >
                      {item.label}
                    </Link>
