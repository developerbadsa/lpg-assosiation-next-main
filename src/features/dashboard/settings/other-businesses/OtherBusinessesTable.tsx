...
import {
  ...
  useDeleteOtherBusiness,
  useOtherBusinesses,
  useUpdateOtherBusiness,
} from './queries';
import OtherBusinessFormModal from './OtherBusinessFormModal';
...
export default function OtherBusinessesTable() {
  const q = useOtherBusinesses();
  const del = useDeleteOtherBusiness();
  const createM = useCreateOtherBusiness();
  const updateM = useUpdateOtherBusiness();

  const [open, setOpen] = useState(false);
  const [mode, setMode] = useState<'create' | 'edit'>('create');
  const [activeRow, setActiveRow] = useState<OtherBusinessRow | null>(null);
  const [formError, setFormError] = useState('');

  const columns = useMemo<ColumnDef<OtherBusinessRow>[]>(() => {
    return [
      {
        id: 'sl',
        header: 'SL#',
      ...
      },
      {
        ...
        sortable: true,
        sortValue: (r) => r.name,
        csvHeader: 'Business Name',
        csvValue: (r) => r.name,
        cell: (r) => <span className="text-[#2B3A4A]">{r.name}</span>,
      },
      {
        id: 'edit',
        header: 'Edit',
  ...
  return (
    <div className="space-y-4">
      ...
      <TablePanel<OtherBusinessRow>
        rows={q.data ?? []}
        columns={columns}
        getRowKey={(r) => r.id}
        searchText={(r) => `${r.sl} ${r.name}`}
        showExport={false}
        totalLabel={(total) => (
          <div className="text-[14px] font-semibold text-[#2D8A2D]">
            Total Other Businesses : <span className="text-[#133374]">{total}</span>
          </div>
        )}
        controlsRightSlot={
          <button
        ...
        onSubmit={async (payload) => {
          ...
          try {
            if (mode === 'create') {
              ...
              if (!activeRow?.id) {
                setFormError('Invalid other business id');
                return;
              }
            ...
            setFormError(e?.message ?? 'Failed to save other business');
          }
        }}
      />
    </div>
